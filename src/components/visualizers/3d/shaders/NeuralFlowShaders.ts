// src/components/visualizers/3d/shaders/NeuralFlowShaders.ts v2.3.8
export const neuralFlowVertexShader = `
  ...
  void main() { 
    vec3 pos = position; 
    float noise = snoise(pos * 0.02 + uTime * 0.2); 
    vNoise = noise; 
    vDist = length(pos); 
    float vein = pow(1.0 - abs(noise), 3.0); 
    vec3 curl = cross(normal, vec3(0.0, 1.0, 0.0)) * snoise(pos * 0.03 + vec3(0.0, 10.0, 0.0) + uTime * 0.15); 
    vec3 newPos = pos - normalize(pos) * (1.0 - vein) * (5.0 + uMids * 10.0) * 0.2 + curl * (2.0 + uBass * 8.0) + normal * smoothstep(0.8, 1.0, sin(length(pos) * 0.1 - uBeat * 5.0)) * 3.0 * uBeat; 
...
export const neuralFlowFragmentShader = `
