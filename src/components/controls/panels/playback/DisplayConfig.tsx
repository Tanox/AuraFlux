    ...
    <BentoCard id="panel-playback-display-config" title={t?.player?.supportInfo || "Display Architecture"} className="flex-1">
        <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <CustomSelect label={t?.lyricsStyle} value={settings.lyricsStyle || LyricsStyle.KARAOKE} options={Object.values(LyricsStyle).map(s => ({ value: s, label: lyricsStyles[s as keyof typeof lyricsStyles] || s }))} onChange={(v) => setSettings({...settings, lyricsStyle: v as LyricsStyle})} />
