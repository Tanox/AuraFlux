// File: app/components/controls/BottomBar.tsx | Version: v1.9.73
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-1">
                {playlist.length === 0 ? <div className="flex flex-col items-center justify-center h-32 text-black/20 dark:text-white/20"><span className="text-[10px] uppercase font-bold">{t?.common?.empty}</span></div> : playlist.map((tr, idx) => (
                    ...
                    >
                        ...
                        <div className="w-8 h-8 rounded bg-black/5 dark:bg-white/5 overflow-hidden border border-black/5 dark:border-white/5 shrink-0">{(tr as any).albumArtUrl && <img src={(tr as any).albumArtUrl} className="w-full h-full object-cover"/>}</div>
