...
export const AiBackground: React.FC = () => {
  ...
  const handleGenerateAiBg = async () => {
      const mood = (currentSong?.mood_en_keywords || currentSong?.mood || 'Vibrant, Abstract, Cosmic') as string;
      
      if (!checkAiServiceAvailability((msg) => showToast(msg, 'error'))) return;
      
      setIsGenerating(true);
      try {
          const imgUrl = await generateArtisticBackground(mood as string);
          if (imgUrl) {
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center h-full">
                    <div className="shrink-0 flex gap-2">
                        ...
                        >
                            ...
                            <span>{isGenerating ? (t?.('common.processing') as string) : ((settings.aiBgUrl ? (t?.('visualPanel.regenerate') as string) : (t?.('visualPanel.generateBg') as string)))}</span>
                        </button>
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <Slider label={t?.('visualPanel.opacity')} value={(settings.aiBgOpacity as number) ?? 0.5} min={0} max={1} step={0.05} onChange={(v)=>handleVisualSettingChange('aiBgOpacity', v)} />
                            <Slider label={t?.('player.blur')} value={(settings.aiBgBlur as number) ?? 0} min={0} max={60} step={2} onChange={(v)=>handleVisualSettingChange('aiBgBlur', v)} />
                        </div>
                    </div>
                </div>
            </div>
