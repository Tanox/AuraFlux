...
// src/components/controls/text/TextStyleSettings.tsx v2.3.11
        ...
        <BentoCard id="panel-text-style" title={t?.('textPanel.appearance') || "Style & Typography"} className="flex-1">
            <div className="space-y-5">
                <div className="space-y-4">
                    <CustomSelect label={t?.('textFont') || "Font"} value={selectValue as string} options={localizedFonts} onChange={handleFontChange} />
                    {selectValue === 'custom' && (
                ...
                <div className="pt-3 border-t border-black/5 dark:border-white/5 grid gap-4">
                    <Slider label={t?.('textSize') || "Size"} value={(settings.customTextSize as number) ?? 12} min={2} max={60} step={1} onChange={handleSizeChange} />
                    <Slider label={t?.('visualPanel.opacity') || "Opacity"} value={(settings.customTextOpacity as number) ?? 0.5} min={0} max={1} step={0.05} onChange={handleOpacityChange} />
                    <Slider label={t?.('textRotation') || "Rotation"} value={(settings.customTextRotation as number) ?? 0} min={-180} max={180} step={5} onChange={handleRotationChange} unit="掳" />
                </div>
