...
export interface VisualsContextType {
  mode: VisualizerMode;
  setMode: React.Dispatch<React.SetStateAction<VisualizerMode>>;
  colorTheme: string[];
...
export const useVisuals = () => {
  ...
  if (!context) {
    throw new Error('useVisuals must be used within an AppProvider');
  }
  return context;
};
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = React.memo(({ children }) => {
  ...
  const audioState = useAudio({
    settings: visualsState.settings,
  ...
  const fileStatus = useMemo(() => {
    return audioState.playlist.length > 0 ? 'ready' as const : 'none' as const;
  }, [audioState.playlist.length]);

  const fileName = useMemo(() => {
    return audioState.playlist[audioState.currentIndex]?.file.name;
