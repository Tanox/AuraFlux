...
# 检查不一致的命令
grep -r "v2\.3\.[0-9]" src/ openspec/ --include="*.ts" --include="*.tsx" --include="*.md" | grep -v "v2.3.10"
```
...
| 阶段 | 验证项 | 命令 |
|------|--------|------|
| 版本统一 | 无不一致版本号 | `grep -r "v2\.3\.[0-9]" src/ openspec/ | grep -v "v2.3.10"` |
