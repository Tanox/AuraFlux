...
const LANGUAGES: { value: Language; label: string }[] = [
  ...
  { value: 'ar', label: 'العربية' }, { value: 'pt', label: 'Português' } // Added pt
];
...
export const OnboardingOverlay: React.FC<OnboardingOverlayProps> = ({ language, setLanguage, onComplete }) => {
  ...
  return (
    <div id="onboarding-overlay" className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-xl">
      <div id="onboarding-content" className="w-full max-w-2xl bg-[#0a0a0c] border border-white/10 rounded-3xl overflow-hidden shadow-2xl relative flex flex-col min-h-[500px] animate-fade-in-up">
        <div className="p-8 pb-4 text-center z-10">
          ...
          <p className="text-white/40 text-sm font-medium tracking-wide uppercase">{step === 0 ? `v${current.appVersion}` : t.subtitle}</p>
        </div>
