...
export const PlaybackPanel: React.FC = () => {
  const { 
    ...
    importFiles, importFromUrl, importPlaylistFromUrl, playNext, playPrev, isPlaying, togglePlayback,
    currentTime, duration, seekFile, currentSong, clearPlaylist
  } = useAudioContext();
  const { t, showToast } = useUI();
  const { settings, setSettings } = useVisuals();
  const { showLyrics, setShowLyrics, apiKeys } = useAI();
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-stretch">
            ...
            <div className="space-y-6">
                {playlist.length > 0 ? (
                    ...
                    <div className="h-40 rounded-2xl border-2 border-dashed border-black/5 dark:border-white/5 flex flex-col items-center justify-center text-black/10 dark:text-white/10 gap-3">
                        <svg className="w-10 h-10 opacity-20" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2z" /></svg>
      ...
      {/* Column 2: Playlist context (7 cols) */}
      <BentoCard 
        ...
        action={
            <div className="flex items-center gap-2">
                ...
                <button onClick={() => fileInputRef.current?.click()} className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white transition-all text-[10px] font-black uppercase tracking-widest shadow-lg">{t?.player?.add || 'LOCAL'}</button>
            </div>
        }
      >
                ...
                <div className="mb-4 p-4 bg-black/[0.04] dark:bg-white/[0.04] rounded-2xl border border-black/5 dark:border-white/5 animate-fade-in-up">
                    <div className="flex gap-2">
                        ...
                        <button 
                            ...
                            className="px-6 py-2 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest disabled:opacity-30 hover:bg-blue-500 transition-all min-w-[100px]"
                        >
