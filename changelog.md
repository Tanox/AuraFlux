...
## [v2.4.1]
### 🎨 Visual Enhancements
- **Flowing Silk (流光丝)**: Increased line thickness to 10x and smoothed line edges for a more fluid appearance.
- **Ocean Wave (海浪)**: Lowered initial wave height and reduced audio reaction gain for a calmer, more grounded visual effect.

## [v2.4.0]
### ✨ New Features
- **PWA Support**: Added Progressive Web App (PWA) capabilities using `@ducanh2912/next-pwa`.
- **Installability**: Generated `manifest.json` and required PNG icons (192x192, 512x512, maskable) for full compatibility with PWABuilder.
- **Offline Ready**: Configured service worker to cache static assets and enable offline access.
- **Mobile Experience**: Added `appleWebApp` meta tags and `viewport` configuration for native-like full-screen experience on iOS/Android.

## [v2.3.3]
### 🐛 Bug Fixes
- **Chunk Loading**: Implemented an automatic page reload mechanism in `ErrorBoundary` to gracefully recover from Next.js dynamic import "Loading chunk failed" errors.
- **Dynamic Imports**: Corrected the relative import path for `ThreeVisualizer` in `App.tsx` to use the `@/src/...` alias for better module resolution.

## [v2.3.2]
### 🛠️ Fixes & UX
- **Cloud Sync**: Added robust error handling for Firestore permission issues.
- **UI Feedback**: Implemented toast notifications for cloud save/load actions.
- **Permission Guard**: Added a visual warning in the account panel when Firestore security rules are missing.
- **Version Sync**: Synchronized all system components to v2.3.2.

## [v2.3.1]
### ✨ New Features
- **Personal Management**: Added profile settings and data privacy options to the account panel.
- **Cloud Storage**: Implemented cloud sync for application settings using Firebase Firestore.
- **Sync UI**: Added last sync time display and manual save/load buttons.
- **Profile Display**: Enhanced account section to show user display name and avatar.

## [v2.3.0]
### 🛠️ Refactoring & Modularity
*   **Code Splitting**: Refactored large code files (>200 lines) into modular components and hooks.
*   **2D Visualizer Engine**: Split `VisualizerCanvas.tsx` drawing logic into separate mode files in `src/components/visualizers/canvas/modes/`.
*   **Context Architecture**: Modularized `AppContext.tsx` into individual context providers (`UIContext`, `VisualsContext`, `AudioContext`, `AIContext`).
*   **Audio Hook**: Refactored `useAudio.ts` into smaller, specialized hooks (`useMediaDevices`, `usePlaylist`).
*   **Efficiency**: Optimized code structure for better token efficiency and maintainability.

## [v2.2.0]
### 🎨 Visualizer Optimizations
*   **Kinetic Wall**: Increased block spacing for better visual clarity and rhythmic pulsing.
*   **Starfield (Particles)**: Improved 2D starfield with a forward-moving perspective effect.
*   **Redundancy Cleanup**: Removed "Liquid Sphere" redundant entry (merged into Resonance Orb).
*   **Mode Cleanup**: Removed unused visualizer modes: Fluid Curves, Ripples, Spiral, and Resonance Rings.
*   **Locale Updates**: Updated major locale files to reflect the streamlined visualizer mode list.

## [v2.1.0]
### ⚡️ Performance & Stability
*   **Chunk Optimization**: Resolved "Loading chunk failed" timeout errors by reducing the initial payload and optimizing script loading strategies.
*   **Font Optimization**: Removed the unused `Montserrat` font to reduce network requests and initial bundle size.
*   **Script Deferral**: Moved `jsmediatags` and Google Analytics to `lazyOnload` to prioritize critical application chunks.
*   **UX Improvement**: Added a dedicated loading UI to the dynamic `App` import to provide immediate visual feedback during initialization.
*   **Version Synchronization**: Updated all version strings to `v2.1.0`.

## [v2.0.9]
### 🐛 Bug Fixes
*   **Audio Engine**: Improved microphone permission handling. The system now gracefully detects permission denial and provides clear user feedback via toast notifications instead of raw console errors.
*   **Version Check**: Suppressed fetch errors in the background version checker to prevent console clutter during network instability.
*   **Version Synchronization**: Updated all version strings to `v2.0.9`.

## [v2.0.8]
### 🛠️ Auth & Database
*   **Firebase Auth**: Integrated Google and GitHub federated login directly into `AuthProvider`.
*   **Firestore Integration**: Added automatic Firestore document creation for new users upon successful login.
*   **Auth UI**: Updated `LoginButton` and `SystemSettingsPanel` to use the centralized authentication methods from `useAuth`.
*   **Localization**: Added `account` and `signOut` keys to all 12 language files.
*   **Version Synchronization**: Updated all version strings to `v2.0.8`.

## [v2.0.7]
### 🛠️ UI & Auth UX
*   **Landing Page**: Removed the mandatory login card from the root page. The application now boots directly into the visualizer for all users.
*   **Auth Consolidation**: Moved all authentication UI (Email/Password, Google, GitHub) into the **Account** section of the **System Settings** panel.
*   **Account UI**: Implemented a dedicated `AccountSection` in the settings panel with user profile display and sign-out functionality.
*   **Version Synchronization**: Updated all version strings to `v2.0.7`.

## [v2.0.6]
### 🛠️ UI & Auth Hardening
*   **Module Resolution**: Fixed "Module not found" errors by creating missing `Button` and `Input` components in `src/components/ui/` and updating import paths.
*   **Component Library**: Implemented standard `Button` and `Input` components using Tailwind CSS and Radix UI primitives to comply with project design system requirements.
*   **Auth Consolidation**: Consolidated authentication context into `src/components/auth/AuthProvider.tsx` and removed redundant `src/context/AuthContext.tsx`.
*   **Utility Functions**: Added `src/utils/cn.ts` for consistent Tailwind class merging.
*   **Version Synchronization**: Updated all version strings to `v2.0.6`.

## [v2.0.5]
### ⚙️ Configuration & Defaults
*   **Style Theme**: Enabled `cycleColors` by default.
*   **Speed**: Set default `colorInterval` to 5 seconds.
*   **Version Synchronization**: Updated all version strings to `v2.0.5`.

## [v2.0.4]
### 🛠️ UI & Layout Refinements
*   **Account Management**: Moved the login/sign-out entry from the main page to the **Account** section within the **System Settings** panel for a cleaner UI.
